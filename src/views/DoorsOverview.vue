<template>
  <div class="door-overview">
      ...
      <div class="filters">
        <div>
          <b-dropdown :text="chosenFilter" variant="primary">
            <b-dropdown-item v-for="group in doorGroups" v-bind:key="group.id" @click="filterGroups(group)">
    ...
    <Pagination v-bind:list="filteredDoors" v-bind:currentPage="currentPage" v-bind:pageSize="pageSize"
      v-on:page:update="updatePage" class="pages" v-if="this.searchterm.length > 0" />
    <Pagination v-bind:list="doors" v-bind:currentPage="currentPage" v-bind:pageSize="pageSize"
      v-on:page:update="updatePage" class="pages" v-if="this.searchterm.length === 0" />
  </div>
</template>
